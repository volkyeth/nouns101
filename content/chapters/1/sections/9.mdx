---
title: How A Crypto Wallets Work
---

A crypto wallet comprises two components: <Nutshell glossaryEntry="content/glossary/public_key.mdx" children="Public Key" />  & <Nutshell glossaryEntry="content/glossary/private_key.mdx" children="Private Key." />&#x20;

With both keys, you can access your cryptocurrency.&#x20;

![](/assets/5.jpg)

Contrary to popular belief, crypto wallets do not store your crypto. They simply allow you to access your crypto on the blockchain.

**There are two main types of crypto wallet:**

<Nutshell glossaryEntry="content/glossary/software_wallet.mdx" children="Software Wallets" />

<Nutshell children="Hardware Wallet" glossaryEntry="content/glossary/hardware_wallet.mdx" />

Software wallets are most common, typically free to create, and are easily accessible.&#x20;



Hardware wallets



The transaction must be sent to your offline hardware wallet, verified by you, and signed by the device before it can be completed.



which are applications you can download and make use of to set up your virtual wallet.

Software wallets are usually one’s first wallet when entering web3 and beginning to collect NFTs or crypto.&#x20;

There are many types of software wallets (aka <Nutshell children="Hot Wallet" glossaryEntry="content/glossary/hot_wallet.mdx" /> , and it’s important to do research to determine which is best suited for you.&#x20;

**On the next page we'll walk you through creating your first Software Wallet...**
